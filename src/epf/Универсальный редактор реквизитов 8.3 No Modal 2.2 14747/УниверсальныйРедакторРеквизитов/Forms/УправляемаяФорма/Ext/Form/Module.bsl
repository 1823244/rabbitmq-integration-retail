...
Процедура ВыбСсылкаПриИзменении(Элемент)
	ВыбСсылкаПриИзмененииСервер();
	ПрочитатьОбъект();
КонецПроцедуры

&НаСервере
Процедура ВыбСсылкаПриИзмененииСервер()
...
Процедура ПриОткрытии(Отказ)
	...
	ЭтаФорма.Элементы.УдалятьИлиИзменятьЗапрещено.Заголовок = "О Сути времени";
...
Процедура УдалитьОбъект(Команда)
		ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьОбъектЗавершение", ЭтаФорма), "Вы осознаёте ответственность за нарушение ссылочной целостности?
...
&НаКлиенте
Процедура УдалитьОбъектЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура УдалитьОбъектФрагмент(Знач Оповещение)
...
Процедура ВыбСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// [lyay изменения 20.12.2012
	Если НЕ ПростаяФормаВыбора И ЗначениеЗаполнено(ВыбСсылка) Тогда
		Элементы.ВыбСсылка.ВыбиратьТип = Ложь;
		Возврат
	КонецЕсли;
	//]
	...
	СтруктураПараметров = Новый Структура("ВыбСсылка, ПростаяФормаВыбора", ПредыдущаяВыбСсылка, ПростаяФормаВыбора);
	...
	ДопПараметры.Вставить("ПредыдущаяВыбСсылка",ПредыдущаяВыбСсылка);
	...
	Иначе
		...
		ОткрытьФорму("ВнешняяОбработка.УниверсальныйРедакторРеквизитов.Форма.УправляемаяФормаВыбораВидаОбъекта",
			СтруктураПараметров,ЭтаФорма.Элементы.ВыбСсылка,,,,
				Новый ОписаниеОповещения("ОткрытиеФормыВыбораКолБэк",ЭтотОбъект,ДопПараметры),
...
//	Параметр1 	- Тип1 - 
//
Процедура ОткрытиеФормыВыбораКолБэк(РезультатВыбораТипа, ДопПараметры) Экспорт
		
	Если РезультатВыбораТипа <> Неопределено Тогда
		Элементы.ВыбСсылка.ОграничениеТипа = Новый ОписаниеТипов(РезультатВыбораТипа.Тип + "Ссылка." + РезультатВыбораТипа.Вид);
		ВыбСсылка = Элементы.ВыбСсылка.ОграничениеТипа.ПривестиЗначение(ВыбСсылка);
		
		ФормаВыбора = ПолучитьФорму(РезультатВыбораТипа.Тип + "." + РезультатВыбораТипа.Вид + ".ФормаВыбора", , ЭтаФорма.Элементы.ВыбСсылка);
		ФормаВыбора.Открыть();
	Иначе
		ВыбСсылка = ДопПараметры.ПредыдущаяВыбСсылка;
	КонецЕсли;
...
Процедура ПоказыватьТолькоРегистрыСДвижениямиПриИзменении(Элемент)
	ПоказыватьТолькоРегистрыСДвижениямиПриИзмененииСервер();
	ПрочитатьОбъект();
КонецПроцедуры

&НаСервере
Процедура ПоказыватьТолькоРегистрыСДвижениямиПриИзмененииСервер()
...
//	ЗапуститьПриложение("http://osutivremeni.ru/");
КонецПроцедуры
...
&НаКлиенте
Процедура ПросмотрБД(Команда)
	СтруктураПараметров = Новый Структура("РежимЗаписиЗагрузка", РежимЗаписиЗагрузка);
	ФормаВыбораВида = ПолучитьФорму("ВнешняяОбработка.УниверсальныйРедакторРеквизитов.Форма.УправляемаяФормаВыбораВидаОбъекта", СтруктураПараметров, , "Просмотр", ВариантОткрытияОкна.ОтдельноеОкно);
	ФормаВыбораВида.ЗакрыватьПриЗакрытииВладельца = Ложь;
	ФормаВыбораВида.ЗакрыватьПриВыборе = Ложь;
	ФормаВыбораВида.Открыть();
КонецПроцедуры

&НаКлиенте
Процедура ПростаяФормаВыбораПриИзменении(Элемент)
	ПростаяФормаВыбораПриИзмененииСервер();
КонецПроцедуры

&НаСервере
Процедура ПростаяФормаВыбораПриИзмененииСервер()
