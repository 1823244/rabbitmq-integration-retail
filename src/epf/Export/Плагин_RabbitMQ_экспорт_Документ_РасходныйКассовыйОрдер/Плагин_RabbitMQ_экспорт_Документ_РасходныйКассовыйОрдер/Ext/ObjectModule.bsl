...
Функция ВыгрузитьОбъект(ДанныеСсылка) Экспорт
	Если ТипЗнч(ДанныеСсылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
	...
	definition.Вставить("НомерСумки", Обк.НомерСумки);
	...
	definition.Вставить("ПлатежнаяВедомость", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(Обк.ПлатежнаяВедомость));
	...
	Для сч = 0 По обк.РасшифровкаПлатежа.Количество()-1 Цикл
		...
		НовСтр.Вставить("ОперацияКассовойОтчетности", ксп_ЭкспортСлужебный.СоздатьУзелIdentification(стрк.ОперацияКассовойОтчетности));
	...
	ТЧОписьСдаваемыхНаличныхДенег = Новый Массив;

	Для сч = 0 По обк.ОписьСдаваемыхНаличныхДенег.Количество()-1 Цикл

		стрк = обк.ОписьСдаваемыхНаличныхДенег[сч];

		НовСтр = Новый Структура;

		НовСтр.Вставить("КоличествоБанкнот", стрк.КоличествоБанкнот);
		НовСтр.Вставить("Коэффициент", стрк.Коэффициент);
		НовСтр.Вставить("НаименованиеБанкноты", стрк.НаименованиеБанкноты);
		НовСтр.Вставить("Номинал", стрк.Номинал);
		НовСтр.Вставить("ПараметрКоличествоБанкнот", стрк.ПараметрКоличествоБанкнот);
		НовСтр.Вставить("ПараметрСуммаБанкнот", стрк.ПараметрСуммаБанкнот);
		НовСтр.Вставить("Сумма", стрк.Сумма);
		ТЧОписьСдаваемыхНаличныхДенег.Добавить(НовСтр);

	КонецЦикла;

	definition.Вставить("ТЧОписьСдаваемыхНаличныхДенег", ТЧОписьСдаваемыхНаличныхДенег);
