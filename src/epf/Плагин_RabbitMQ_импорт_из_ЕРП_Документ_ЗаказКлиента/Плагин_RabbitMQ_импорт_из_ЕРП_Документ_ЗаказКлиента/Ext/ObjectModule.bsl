...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	Для счТовары = 0 По деф.ТЧТовары.Количество()-1 Цикл
		...
		НовСтр.Номенклатура = ксп_ИмпортСлужебныйПовтИсп.НайтиНоменклатуру(стрк.Номенклатура);
		
		НовСтр.Отменено = стрк.Отменено;	
		
		НовСтр.ПричинаОтмены = РегистрыСведений.ксп_МэппингПричинаОтменыЗаказаКлиента.ПоМэппингу(стрк.ПричинаОтмены);
		...
		НовСтр.Характеристика = ксп_ИмпортСлужебныйПовтИсп.НайтиХарактеристику(стрк.Характеристика);
	
		НовСтр.Цена = стрк.Цена;
		
		
		Упаковка = ксп_ИмпортСлужебныйПовтИсп.НайтиЕдиницуИзмерения(стрк.Упаковка, стрк.Номенклатура);
...
Функция РезервироватьИлиНет(ВариантОбеспечения)
	...
	Если ВариантОбеспечения.Значение = "СоСклада " Тогда
		Возврат Истина;
	ИначеЕсли ВариантОбеспечения.Значение = "Отгрузить " Тогда
		Возврат Истина;
	Иначе 
		Возврат Ложь;
	КонецЕсли;
