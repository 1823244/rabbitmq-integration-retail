...
Процедура ВариантОтбораПоНомеруСообщенияПриИзменении(Элемент)
	УстановитьОтборПоНомеруСообщенияКл(СписокКонстант,       ВариантОтбораПоНомеруСообщения);
	УстановитьОтборПоНомеруСообщенияКл(СписокСсылок,         ВариантОтбораПоНомеруСообщения);
	УстановитьОтборПоНомеруСообщенияКл(СписокНаборовЗаписей, ВариантОтбораПоНомеруСообщения);
	ОбновитьСодержимоеСтраницы();
КонецПроцедуры
...
Процедура ДобавитьРегистрациюВСписокСсылок(ЭтоПодбор = Ложь)
	...
	ТекИмяФормы = СтрЗаменить(ПолучитьИмяФормы("СписокСсылок"),".Форма",".ФормаВыбора");
...
Функция ПолучитьИмяФормы(ТекущийОбъект = Неопределено)
	...
	Если не ТекущийОбъект = Неопределено Тогда
		Возврат ЭтотОбъект[ТекущийОбъект].ОсновнаяТаблица+".Форма.";
	Иначе
...
&НаКлиенте
Процедура УстановитьОтборПоНомеруСообщенияКл(ДинамоСписок, Вариант)
