...
Функция ЗагрузитьОбъект(СтруктураОбъекта, jsonText = "") Экспорт
	...
	// надо прогрузить Розничного покупателя
	...
	// Предполагается, что в мэппинге будет ограниченное число записей, например
	// для Розничного покупателя и, может быть, еще несколько служебных,
	// а основная работа будет вестись по ГУИДу
	// В мэппинг элементы будут добавляться вручную
	
	ПоМэппингу = Неопределено;
	Если РегистрыСведений.ксп_МэппингСправочникКонтрагенты.ЕстьГУИД(id.Ref, мВнешняяСистема) Тогда
		ПоМэппингу = РегистрыСведений.ксп_МэппингСправочникКонтрагенты.ПоМэппингу(id.Ref, мВнешняяСистема);
	Иначе 
		// Не добавляем ГУИД автоматически, это делается вручную!
		//РегистрыСведений.ксп_МэппингСправочникКонтрагенты.ДобавитьГУИД(id.Ref, def.Description+", Код: "+id.code);
	...
	Иначе 
		Если id.isFolder Тогда
			ОбъектДанных = Справочники.Контрагенты.СоздатьГруппу();
		Иначе 
