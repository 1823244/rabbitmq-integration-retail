...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, 
		"Выполнить создание перемещений по данным Заказа на перемещение и Расходного ордера на товары",
		"СозданиеПеремещений",
		ТипКоманды, 
		Ложь) ;
...
Функция СоздатьПеремещениеТоваров(ЗаказНаПеремещениеГУИД,РасходныйОрдерГУИД, ПеремещениеСсылка)
	...
	Попытка
		...
		Обк.Дата = ДанныеЗнП.ДатаДокумента;
		Обк.Номер = ДанныеЗнП.НомерДокумента;      //2024-01-29
		...
		Если не ЗначениеЗаполнено(Обк.ОрганизацияПолучатель) Тогда
			Обк.ОрганизацияПолучатель = Обк.Организация;
		КонецЕсли;
		...
		ксп_ИмпортСлужебный.ДобавитьОтложенноеПроведение(Обк.Ссылка);
	...
	Исключение
	    ...
	    ЗаписьЖурналаРегистрации("СозданиеПеремещенийТоваровЕРП",УровеньЖурналаРегистрации.Ошибка,,,
			"НЕ удалось создать перемещение. Подробности: "+тдебаг);
		
	    //Сообщить(НСтр("ru = '"+ОписаниеОшибки()+"'"), СтатусСообщения.Внимание);
	КонецПопытки;
	
	// пока отключено, т.к. работает Отложенное проведение
