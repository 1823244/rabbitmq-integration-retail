...
Процедура ВыполнитьИмпорт_НСИ() Экспорт
	...
	Пока Клиент.BasicConsumeMessage("", ПолучаемоеСообщение, ТегСообщения, 5000) Цикл
		...
		Исключение                                                                 
			...
			Клиент.BasicReject(ТегСообщения);  // удаляет из очереди
		...
		Если НЕ Успешно Тогда
			...
			Попытка
				//Клиент.BasicAck(ТегСообщения); // не удаляет из очереди
			...
			Исключение           
				т = Клиент.GetLastError();
				ЗаписьЖурналаРегистрации("ИмпортИзЕРП",УровеньЖурналаРегистрации.Ошибка,,,
				"Ошибка RabbitMQ при пропуске (BasicReject без загрузки в 1С, т.к. не найден нужный плагин) сообщения с тэгом <"+Строка(ТегСообщения)+">. Сессия ИД "+ИДСессии+". Подробности: "+т);
